    ...
    case STATE_RUNNING:
      ...
      if (wdtSleepCount == 15)
      {
        ...
        else
        {
          ...
          // short beep to indicate running
          shortBeep();       
...
void playAlarm()
{
  ...
  while (((millis() - alarmTimer) < MAX_ALARM_TIME) || (pressedOnOff))
  {
    ...
    while (((millis() - pauseTimer) < ALARM_PAUSE_TIME) || (pressedOnOff))
    {
      // test for power button
      if (onOffButton.update())
      {
